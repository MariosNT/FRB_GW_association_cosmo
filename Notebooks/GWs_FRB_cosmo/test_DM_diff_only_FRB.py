...
RESUME = True
CKP_INTERVAL = 50
SAVE_FILE = './test_DM_diff_checkpoint/FRB_simulation_data.pkl'
MCMC_FILE = './test_DM_diff_checkpoint/FRB_mcmc_checkpoint.pkl'
...
Path('./test_plot').mkdir(parents=True, exist_ok=True)
plt.savefig('./test_plot/data_only_FRB_DM_diff.png')
# plt.tight_layout()
...
def log_likelihood(theta, zs, dLs, s_dLs, DMs, s_DMs):
    ...
    try:
        for idx, (z, dL_obs, s_dL, DM_obs, s_DM) in enumerate(zip(zs, dLs, s_dLs, DMs, s_DMs)):
            ...
            p_event = normalise(p_DM, z_array)
...
if __name__ == '__main__':
    ...
    mcmc_plot_results(samples, param_names, savetitle='./test_plot/MCMC_only_FRB_DM_diff')